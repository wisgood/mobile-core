 ...
 * www.funshion.com
 *
 * @Title: DbufferQualityFormatFilterByError.java 
...
/**
 * @ClassName: DbufferQualityFormatFilterByError
 * @Description: 这里用一句话描述这个类的作用
 * @author fuys
 * @date 2013-5-10 下午2:59:19
 */
public class DbufferQualityFormatFilterByErrorMR {

    public static class DbufferQualityFormatFilterByErrorMap extends
        ...
        private static Logger logger = Logger
                .getLogger(DbufferQualityFormatFilterByErrorMap.class.getName());
        ...
        public Map<String, String> getDbufferQualityFormatFilterByErrorStr(
                String originalData) {
            // PlayTMOtherETL playTMETL = null;
            Map<String, String> dbufferQualityFormatMap = new WeakHashMap<String, String>();
            String[] splitSts = originalData.split(",");
            if (splitSts.length <= DbufferEnum.BTM.ordinal()) {
                dbufferQualityFormatMap.put("colcum lenght less "
                        + (DbufferEnum.BTM.ordinal() + 1), originalData);
            }
            else {
                try {
...
//                    String cityId = ipRuleMap.get(ConstantEnum.CITY_ID);
                    ...
                    long btmlong = FormatMobileUtil.parseDoubleToLong(splitSts,
                            DbufferEnum.class.getName(),
                            DbufferEnum.BTM.toString());
                    long bposlong = FormatMobileUtil.parseDoubleToLong(
                            splitSts, DbufferEnum.class.getName(),
                            DbufferEnum.BPOS.toString());

                    String ipFormatStr = IPFormatUtil.ipFormat(ipInfoStr);

                    String serverIpStr = splitSts[DbufferEnum.SERVERIP
                            .ordinal()];
                    long serverId = 0;
                   ...
                   // logger.error(e.getMessage(), e.getCause());
                    dbufferQualityFormatMap.put(e.getMessage(), originalData);
                }
            }
            return dbufferQualityFormatMap;
        }

        public void map(LongWritable key, Text value, Context context)
                throws IOException, InterruptedException {
            String line = value.toString();
            Map<String,String> dbufferQualityFormatMap = this.getDbufferQualityFormatFilterByErrorStr(line);
            if (null != dbufferQualityFormatMap && dbufferQualityFormatMap.size() > 0) {
                Set<Map.Entry<String, String>> set = dbufferQualityFormatMap.entrySet();
    ...
    public static class DbufferQualityFormatFilterByErrorReduce extends
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJobName("DbufferQualityFormatFilterByError");
        ...
        FileOutputFormat.setOutputPath(job, new Path("output_dbuffer_quality_errror"));
