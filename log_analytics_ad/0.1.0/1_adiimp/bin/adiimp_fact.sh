...
echo "adiimp start at $(date +%T)"
#check output dirctory
output_dir=$HDFS_FACT_DIR/f_ad_hermes_adi_imp_day/$date_slash
...
log_types={deliver,monitor,click}
input_dirs=""
warnings=""
...
#rum mapreduce 
$INSTALL_HADOOP/hadoop jar jar/AD_DW.jar com.bi.ad.fact.adiimp.ADIIMPMR                   \
                                            -files files/area_mapping_info,files/adp_info \
                                            -D input_dir=$input_dirs                      \
                                            -D output_dir=$output_dir                     \
                                            -D reduce_num=30                              \
                                            -D stat_date=$date                            \
                                            -D job_name="ADIIMPMR-$date" 2>&1
...
${KETTLE_HOME}/kitchen.sh  -file=kettle/ADI_IMP_JOB.kjb                            \
                           -level=Error                                                 \
                           -param:specified_ktr=kettle/F_AD_HERMES_ADI_IMP_DAY.ktr \
                           -param:date=${date}                                          \
                           -param:db_table_name=F_AD_HERMES_ADI_IMP_DAY                 \
                           -param:input_path="hdfs://192.168.117.20:8020${output_dir}"  \
                           > logs/$date/F_AD_HERMES_ADI_IMP_DAY.log
if [[ $? -ne 0 ]]
then
