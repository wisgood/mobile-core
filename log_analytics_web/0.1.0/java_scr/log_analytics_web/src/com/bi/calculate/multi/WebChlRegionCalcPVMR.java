...
public class WebChlRegionCalcPVMR extends Configured implements Tool {

	public static class WebChlRegionCalcPVMapper extends
		...
		public void map(LongWritable key, Text value, Context context)
			...
			for (int i = 0; i < urlTypes.length; i++) {
				if (field[PvFormatEnum.URL_SECOND_ID.ordinal()].equals(urlTypes[i])) {
					isChannel = 1;
					break;
				}
			}
	...
	public static class WebChlRegionCalcPVReduce extends
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(conf, "WebChlRegionCalcPVMR");
		job.getConfiguration().set("column", args[2]);
		job.setJarByClass(WebChlRegionCalcPVMR.class);
		FileInputFormat.setInputPaths(job, new Path(args[0]));
		FileOutputFormat.setOutputPath(job, new Path(args[1]));
		job.setMapperClass(WebChlRegionCalcPVMapper.class);
		job.setReducerClass(WebChlRegionCalcPVReduce.class);
	...
	public static void main(String[] args) throws Exception {
		...
		int nRet = 0;
		try {
			logArgs.init("WebChlRegionCalcPVMR.jar");
			logArgs.parse(args);
		} catch (Exception e) {
		...
		nRet = ToolRunner.run(new Configuration(), new WebChlRegionCalcPVMR(),
