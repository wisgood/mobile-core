...
public class MediaHistoryCountMR extends Configured implements Tool{

	public static class MediaHistoryCountMap extends
				...
				if (field.length == 4) {
					...
					for(int i = 0; i < urlTypes.length; i++){
						if(field[PlayFormatEnum.REFER_SECOND_ID.ordinal()].equals(urlTypes[i])){
							...
							if(mediaId.equals("") || mediaId == null || mediaId.isEmpty()) {
								mediaId = "-1";
							}
	...
	public static class MediaHistoryCountReduce extends Reducer<Text, Text, Text, Text> {
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			if (label) {
				context.write(new Text(historyout.trim()),new Text(""));
			}
			if (!label && exLabel) {
				String[] playInfoStrs = newout.split("\t");
				String mediaout = key.toString();
				
				String newHistoryUserInfoStr = mediaout + "\t"
				...
				context.write(new Text(newHistoryUserInfoStr.trim()),new Text(""));
			}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(conf, "MediahistoryMR");
		//job.getConfiguration().set("column", args[2]);
		job.setJarByClass(MediaHistoryCountMR.class);
		FileInputFormat.setInputPaths(job, args[0]);
		FileOutputFormat.setOutputPath(job, new Path(args[1]));
		job.setMapperClass(MediaHistoryCountMap.class);
		job.setReducerClass(MediaHistoryCountReduce.class);
	...
	public static void main(String[] args) throws Exception {
		...
		int nRet = 0;
		try {
			logArgs.init("MediaHistoryCountMR.jar");
			logArgs.parse(args);
		} catch (Exception e) {
		...
		nRet = ToolRunner.run(new Configuration(), new MediaHistoryCountMR(),
