...
/**
 * 
 * @ClassName: LandPageOutBR
 * @Description: 计算着陆页相关指标,如PV,UV,SessionCount等,但不包括跳出率(Bounce Rate)
 * @author liuyn
 * @date July 25, 2013 AM
 */

public class LandPageOutBR extends Configured implements Tool {

    private static final String SEPARATOR = "\t";

    public static class LandPageOutBRMapper extends
        ...
        public void map(LongWritable key, Text value, Context context)
            ...
            if (referThirdId > 1000 && "1".equals(urlFirstIdStr)) {
                context.write(new Text(dateIdStr + SEPARATOR + StringDecodeFormatUtil.urlNormalizer(urlStr)),
                        new Text(fckStr + SEPARATOR + sessionIdStr));
            }
        }
    ...
    public static class LandPageOutBRReducer extends
    ...
    public static void main(String[] args) throws Exception {
        ...
        nRet = ToolRunner.run(new Configuration(), new LandPageOutBR(),
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = new Job(conf, "LandPageOutBR");
        job.setJarByClass(LandPageOutBR.class);
        for (String path : args[0].split(",")) {
            FileInputFormat.addInputPath(job, new Path(path));
        }

        FileOutputFormat.setOutputPath(job, new Path(args[1]));
        job.setMapperClass(LandPageOutBRMapper.class);
        job.setReducerClass(LandPageOutBRReducer.class);
