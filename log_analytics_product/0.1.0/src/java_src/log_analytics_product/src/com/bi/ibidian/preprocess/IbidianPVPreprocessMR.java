...
public class IbidianPVPreprocessMR extends Configured implements Tool
{
	public static class IbidianPVPreprocessMap extends Mapper<LongWritable, Text, Text, Text>
	{
				...
				InterruptedException
		{
			...
			{
				//context.write(
				//		new Text(originRecord.split(",")[IbidianPVEnum.O_TIMESTAMP.ordinal()]),
				//		new Text(originRecord));
			}
			else
			{
				context.write(new Text(
						formatRecord.split("\t")[IbidianPVEnum.O_TIMESTAMP.ordinal()]), new Text(
						formatRecord));
			}
		}
		...
		private String getFormatRecord(String originRecord)
		{
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_TIMESTAMP.ordinal()].trim();
				try
				{
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_IP.ordinal()].trim();
				try
				{
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_FCK.ordinal()].trim();
				if ("".equals(tmpStr))
				{
					oFCK = "UNDEFINED";
				}
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_MAC.ordinal()].trim();
				if ("".equals(tmpStr))
				{
					oMAC = "UNDEFINED";
				}
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_USERID.ordinal()].trim();
				if ("".equals(tmpStr))
				{
					oUserID = -1;
				}
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_PAGENAME.ordinal()].trim();
				try
				{
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_BLOCKNAME.ordinal()].trim();
				try
				{
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_URL.ordinal()].trim();
				try
				{
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_REFER.ordinal()].trim();
				try
				{
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_ORDER.ordinal()].trim();
				if ("".equals(tmpStr))
				{
					oOrder = -1;
				}
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_TIME.ordinal()].trim();
				if ("".equals(tmpStr))
				{
					oTime = -1;
				}
			...
			try
			{
				tmpStr = splitsOriginRecord[IbidianPVEnum.O_CORE.ordinal()].trim();
				if ("".equals(tmpStr))
				{
					oCore = -1;
				}
			...
			formatRecordBuffer.append(oOrder + "\t");
			formatRecordBuffer.append(oTime + "\t");
			formatRecordBuffer.append(oCore + "\t");
	...
	public static class IbidianPVPreprocessReduce extends Reducer<Text, Text, Text, Text>
	{
		@Override
	...
	public int run(String[] argv) throws Exception
	{
		...
		Job job = new Job(conf, "IbidianPVPreprocessMR");
		job.setJarByClass(IbidianPVPreprocessMR.class);
		FileInputFormat.setInputPaths(job, new Path(argv[0]));
		FileOutputFormat.setOutputPath(job, new Path(argv[1]));
		job.setMapperClass(IbidianPVPreprocessMap.class);
		job.setReducerClass(IbidianPVPreprocessReduce.class);
	...
	public static void main(String[] argv) throws Exception
	{
		...
		try
		{
			mrArgs.initParserOptions("ibidianpvpreprocessmr.jar");
		...
		int nRet = 0;
		nRet = ToolRunner.run(new Configuration(), new IbidianPVPreprocessMR(),
