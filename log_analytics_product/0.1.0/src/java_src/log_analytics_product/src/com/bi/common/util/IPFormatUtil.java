...
public class IPFormatUtil {
    ...
    public static String getVersionFormActionObjser(String actionobserStr) {
        StringBuilder dateIdSB = new StringBuilder();
        String regEx = "(\\d+\\.\\d+\\.\\d+\\.\\d+)";

        Pattern pat = Pattern.compile(regEx);
        Matcher mat = pat.matcher(actionobserStr);
        while (mat.find()) {
            for (int i = 1; i <= mat.groupCount(); i++) {
                dateIdSB.append(mat.group(i));
            }
        }
        return dateIdSB.toString();
    }

    public static String getVersionFormActionObjser(String screendllStr,
            String actionobserStr) {
        StringBuilder dateIdSB = new StringBuilder();
        actionobserStr = actionobserStr.toLowerCase();
        screendllStr = screendllStr.toLowerCase();
        if (actionobserStr.contains(screendllStr)) {
            ...
            String versionStr = dateIdSB.toString();
            if ("".equalsIgnoreCase(versionStr.trim())) {
                versionStr = "0.0.0.0";
