 ...
 * 
 */
public class CDByColsLlyMR_TASKSTAT_TASKNUM extends Configured implements Tool
{
			...
			Mapper<LongWritable, Text, Text, LongWritable>
	{
				...
				InterruptedException
		{
			...
			if ("TaskStatEnum".equals(this.enumName))
			{
				...
				try
				{
					ftd = Long.parseLong(this.formatFields[TaskStatEnum.FTD.ordinal()]);
					ftp = Long.parseLong(this.formatFields[TaskStatEnum.FTP.ordinal()]);
					ftsd = Long.parseLong(this.formatFields[TaskStatEnum.FTSD.ordinal()]);
				}
				...
				long taskNum = ftd + ftp + ftsd;

				keysValueBuffer.append(dateID);
				keysValueBuffer.append("\t");
				keysValueBuffer.append(taskNum);
				keysValueBuffer.append("\t");
	...
	public int run(String[] argv) throws Exception
	{
		...
		String mrPath = CDByColsLlyMR_TASKSTAT_TASKNUM.class.getName();
		int lastDotPos = mrPath.lastIndexOf(".") + 1;
		String mrName = mrPath.substring(lastDotPos);

		job.setJobName(mrName);
		job.setJarByClass(CDByColsLlyMR_TASKSTAT_TASKNUM.class);
	...
	public static void main(String[] argv) throws Exception
	{
		...
		int nRet = 0;
		nRet = ToolRunner.run(new Configuration(), new CDByColsLlyMR_TASKSTAT_TASKNUM(),
